...
// Per-task convolution (same AVX2 + OpenMP kernel as compute_optimized.c).
//
// In the MPI build, the *coordinator* (staff-provided) distributes whole tasks
// across MPI ranks and calls execute_task -> convolve on each rank. The
// convolution of a single task is therefore identical to the shared-memory
...
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  ...
  uint32_t simd_end = b_cols - (b_cols % 8);
